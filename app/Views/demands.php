...
<html lang="en">
    ...
    <body>
        ...
        <button type="button" class="btn btn-success mb-2 addButton" data-bs-toggle="modal" data-bs-target="#addModal">Add New</button>
        <br>
...
<style>
    ...
    table#demands.dataTable tbody tr.highlight {
        background-color: #7cffcb;
        background-image: linear-gradient(315deg, #7cffcb 0%, #74f2ce 74%);
    }
     
    table#demands.dataTable tbody tr.highlight > .sorting_1 {
        background-color: #7cffcb;
        background-image: linear-gradient(315deg, #7cffcb 0%, #74f2ce 74%);
    }
</style>
...
<script>
    ...
    $(document).on("click", "#demands tr td .editButton", function () {
        var rowIndex = $(this).closest('tr');
        console.log(rowIndex);
        var id = $("#demands").DataTable().row(rowIndex).index();
        $("#editForm").data('rowID', '');
        $("#editForm").data('rowID', id);
        console.log("ROW = " + id);
        ...
        httpRequest.onload = function() {
            ...
            $(".form-group #ibhaan_spoc2").val( ib ).change();
    ...
    $("#ibhaan_spoc").on('change', function(){
        var spoc = $(this).val();
        var users = <?php echo json_encode($userNames)?>;
        for(var userIndex in users)
        {
            if(users[userIndex].full_name.toLowerCase() == spoc.toLowerCase())
            {
                console.log(spoc.toLowerCase(), users[userIndex].full_name.toLowerCase())
                $('.form-group #recruiter').val(users[userIndex].user_id);
            }
        }
    })

    $("#ibhaan_spoc2").on('change', function(){
        var spoc = $(this).val();
        var users = <?php echo json_encode($userNames)?>;
        for(var userIndex in users)
        {
            ...
            {
                console.log(spoc.toLowerCase(), users[userIndex].full_name.toLowerCase())
                $('#recruiter2').val(users[userIndex].user_id);
            }
        }
    })
        ...
        $.ajax({
            ...
            success: function(data)
            {
                ...
                if(str[0].valueOf() === "success".valueOf())
                {
                    ...
                    var index = $("#editForm").data('rowID');
                    $("#demands").DataTable().row(index).data(str[3][0]);
                    $( $("#demands").DataTable().rows().nodes() ).removeClass( 'highlight' );
                    $( $("#demands").DataTable().row(index).nodes() ).addClass( 'highlight' );
                    console.log($("#demands").DataTable().row(index).nodes());
                    const timeoutFunc = setTimeout(removeHighlight, 2000);
    ...
    $(document).ready(function() {
        ...
        columns.push({ data: null, title: "EDITS", className: "text-center justify-content-center",
